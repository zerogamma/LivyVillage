...
import reducer, { fetchVillage , fetchSuccessful ,fetchPending , fetchFailed} from '../store/villageList'
...
describe("Redux function", () => {
    describe('reducer, actions and selectors', () => {
        ...
        it('creates fetchSuccessful when data fetch succeeds ', async () => {
          const initialState =  { entities: [] , currentPage : 0 , noData: true , loading: false};
        ...
        it('creates fetchFailed when data fetch fails', async () => {
            ...
            const getError = store.getActions().map( action => action.type === 'users/fetchVillage/rejected' ? action.error.message : null)
            const expectedError = expectedActions.map( action => action.type === 'users/fetchVillage/rejected' ? action.error.message : null)
